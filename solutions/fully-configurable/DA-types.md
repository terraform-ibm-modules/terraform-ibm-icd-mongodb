# Configuring complex inputs in Databases for MongoDB
...
The Configuration variable tunes the MongoDB database to suit different use case. For more information, see [Configuration](https://cloud.ibm.com/docs/databases-for-mongodb?topic=databases-for-mongodb-changing-configuration&interface=cli).
...
- `maxmemory`: Determines the amount of data that you can store in MongoDB as a percentage of the deployments memory. (default: `80`).
- `maxmemory-policy`: Determines eviction behavior when `maxmemory` limit is reached [Learn more](https://cloud.ibm.com/docs/databases-for-mongodb?topic=databases-for-mongodb-mongodb-cache&interface=cli#mongodb-cache-maxmemory-policy) (default: `noeviction`).
- `appendonly`: Enables MongoDB persistence when set to `yes`, If you are caching data, you want to set this value to `no`. (default: `yes`).
- `maxmemory-samples`: Tunes LRU eviction algorithm when MongoDB is configured as a cache [Learn more](https://cloud.ibm.com/docs/databases-for-mongodb?topic=databases-for-mongodb-mongodb-cache&interface=cli#mongodb-cache-other-settings) (default: `5`).
- `stop-writes-on-bgsave-error`: MongoDB stops accepting writes if it detects an unsuccessful backup snapshot. For caching, you can set to `no`. (default: `yes`).
