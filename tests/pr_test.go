...
func GetVersionsGen2(region string, plan string) (string, string) {
	...
	icdAvailableVersions, err := cloudInfoSvc.GetAvailableIcdVersionsGen2("databases-for-mongodb", plan, region) // this function takes service, plan and region as arguments in this specific order
...
func TestRunFullyConfigurableGen2SolutionSchematics(t *testing.T) {
	...
	options.TerraformVars = []testschematic.TestSchematicTerraformVar{
		...
		// TODO: Change to "enterprise-sharding-gen2" once PR #6923 is merged and the provider version is bumped
...
func TestRunExistingInstance(t *testing.T) {
	...
	if existErr != nil {
		...
		logger.Log(t, "existing_mongodb_instance_crn: ", terraform.OutputContext(t, context.Background(), existingTerraformOptions, "mongodb_crn"))
		options := testschematic.TestSchematicOptionsDefault(&testschematic.TestSchematicOptions{
			Testing: t,
			TarIncludePatterns: []string{
				"*.tf",
		...
		options.TerraformVars = []testschematic.TestSchematicTerraformVar{
			{Name: "prefix", Value: options.Prefix, DataType: "string"},
			{Name: "ibmcloud_api_key", Value: options.RequiredEnvironmentVars["TF_VAR_ibmcloud_api_key"], DataType: "string", Secure: true},
			{Name: "existing_mongodb_instance_crn", Value: terraform.OutputContext(t, context.Background(), existingTerraformOptions, "mongodb_crn"), DataType: "string"},
